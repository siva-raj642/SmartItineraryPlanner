...
import { WeatherService, WeatherData, CurrentWeather } from '../../services/weather.service';
import { PdfExportService } from '../../services/pdf-export.service';
import { PackingListService, PackingItem } from '../../services/packing-list.service';
import { CurrencyService, Currency } from '../../services/currency.service';
import { TranslationService, Language, TranslationResult } from '../../services/translation.service';
...
export class ItineraryDetailsComponent implements OnInit, OnDestroy {
  ...
  autoTranslateNames(): void {
    ...
    console.log('Activities:', this.itinerary.activities.map(a => a.name));

    // Only extract Latin names if target is English
    if (this.selectedLanguage === 'en') {
      this.itinerary.activities.forEach(activity => {
        const extractedLatin = this.translationService.extractLatinName(activity.name);
        if (extractedLatin) {
          console.log(`Extracted Latin from "${activity.name}": "${extractedLatin}"`);
    ...
    const namesToTranslate = this.itinerary.activities
      .map(a => a.name)
      .filter(name => {
        const needsTranslation = !this.translatedNames.has(name) && this.translationService.needsTranslation(name);
    ...
    console.log(`Translating ${uniqueNames.length} names to ${this.selectedLanguage}:`, uniqueNames);
    
    this.translationService.translateMultiple(uniqueNames, this.selectedLanguage).subscribe({
      next: (results: TranslationResult[]) => {
        console.log('Translation results received:', results);
        results.forEach((result: TranslationResult) => {
          console.log(`Setting translation: "${result.original}" → "${result.translated}"`);
          this.translatedNames.set(result.original, result.translated);
        });
        console.log('TranslatedNames map:', Array.from(this.translatedNames.entries()));
        this.translationEnabled = true;
  ...
  getCurrentLanguage(): Language | undefined {
    return this.availableLanguages.find(l => l.code === this.selectedLanguage);
  }
  ...
  loadWeather(): void {
    ...
    // Get current weather
    this.weatherService.getCurrentWeather(this.itinerary.destination).subscribe(weather => {
      this.currentWeather = weather;
    });
    
    // Get forecast
    const startDate = new Date(this.itinerary.start_date).toISOString().split('T')[0];
    const endDate = new Date(this.itinerary.end_date).toISOString().split('T')[0];
    
    this.weatherService.getWeatherForecast(this.itinerary.destination, startDate, endDate).subscribe(forecast => {
  ...
  loadDestinationClock(): void {
    ...
    this.timezoneService.getTimezone(this.itinerary.destination).subscribe(timezone => {
  ...
  updateDestinationTime(): void {
    if (this.destinationTimezone) {
      this.destinationTime = this.timezoneService.getCurrentTimeInTimezone(this.destinationTimezone);
      this.cdr.detectChanges();
    }
  }
  ...
  getUtcOffset(): string {
    if (this.destinationTime) {
      return this.timezoneService.formatUtcOffset(this.destinationTime.utcOffset);
    }
    return '';
  }
  ...
  getTimezoneCityName(): string {
    if (this.destinationTimezone) {
      return this.timezoneService.getTimezoneDisplayName(this.destinationTimezone);
    }
    return '';
  }
  ...
  generatePackingList(): void {
    ...
    this.packingCategories = this.packingListService.getCategories(this.packingItems);
  }
  ...
  getItemsByCategory(category: string): PackingItem[] {
    return this.packingListService.getItemsByCategory(this.packingItems, category);
  }
  ...
  loadCurrencyInfo(): void {
    ...
    this.targetCurrency = this.currencyService.getCurrencyForDestination(this.itinerary.destination);
  ...
  updateExchangeRate(): void {
    this.currencyService.getExchangeRate(this.baseCurrency, this.targetCurrency).subscribe(rate => {
  ...
  calculateConversion(): void {
    this.convertedAmount = this.currencyService.convert(this.convertAmount, this.exchangeRate);
  }
  ...
  async exportToPdf(): Promise<void> {
    ...
    await this.pdfExportService.exportItineraryToPdf(this.itinerary, this.groupedActivities, travelerName);
  }
  ...
  groupActivitiesByDay(): void {
    ...
    const activitiesPerDay = Math.ceil(this.itinerary.activities.length / duration);
    ...
    for (let day = 1; day <= duration; day++) {
      ...
      const endIndex = Math.min(startIndex + activitiesPerDay, this.itinerary.activities.length);
      const dayActivities = this.itinerary.activities.slice(startIndex, endIndex);
      ...
      if (dayActivities.length > 0) {
        this.groupedActivities.push({
          ...
          totalCost: dayActivities.reduce((sum, a) => sum + (a.estimatedCost || 0), 0)
        });
      }
    }
  }
  ...
  calculateTotalExpenses(): number {
    ...
    return this.itinerary.activities.reduce((sum, activity) => sum + (activity.estimatedCost || 0), 0);
  }
  ...
  deleteItinerary(): void {
    if (!this.itinerary?.id || !confirm('Are you sure you want to delete this itinerary?')) {
      return;
    }
  ...
  regenerateActivities(): void {
    ...
    if (!confirm('This will fetch fresh places from the API and replace your current activities. Continue?')) {
      return;
    }
    ...
    this.itineraryService.regenerateActivities(this.itinerary.id).subscribe({
      next: (response) => {
        ...
        alert('Activities refreshed successfully with real places from the API!');
      },
      error: (error) => {
  ...
  shareItinerary(): void {
    ...
    const shareData = {
      ...
      text: `Check out my travel itinerary to ${this.itinerary.destination} from ${new Date(this.itinerary.start_date).toLocaleDateString()} to ${new Date(this.itinerary.end_date).toLocaleDateString()}!`,
  ...
  private copyToClipboard(shareData: { title: string; text: string; url: string }): void {
  ...
  translateAllActivities(): void {
    ...
    const namesToTranslate = this.itinerary.activities
      .map(a => a.name)
      .filter(name => !this.translatedNames.has(name) && this.translationService.needsTranslation(name));
  ...
  getDisplayName(activity: Activity): string {
    ...
    console.log(`getDisplayName("${activity.name}"): has translation = ${hasTranslation}`);
    ...
    const extractedEnglish = this.translationService.extractEnglishName(activity.name);
    if (extractedEnglish) {
  ...
  hasNonEnglishActivities(): boolean {
    ...
    const hasNonEnglish = this.itinerary.activities.some(a => this.translationService.needsTranslation(a.name));
    console.log('hasNonEnglishActivities:', hasNonEnglish);
    return hasNonEnglish;
  }

  isTranslated(activity: Activity): boolean {
    return this.translatedNames.has(activity.name) && 
  ...
  getMediaFiles(): string[] {
    ...
    const mediaPaths = this.itinerary.media_paths as unknown;
    if (typeof mediaPaths === 'string') {
      return (mediaPaths as string).split(',').filter((p: string) => p.trim().length > 0);
    }
